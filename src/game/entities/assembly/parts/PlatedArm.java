...
public class PlatedArm extends Part implements Shoot{
    ...
    public void shoot(long time){
        if(time-this.startedShooting>this.shootCooldown&this.owner.getEnergy()>=10){
            ...
            this.owner.triggerShoot(new LaserProjectile((this.facingRight?1:-1)*this.subParts[2].getOrigin().getX()+this.owner.getX(), this.subParts[2].getOrigin().getY()+this.owner.getY(), this.owner.getRoom(), GameScene.getMouseX(),GameScene.getMouseY(), State.CYAN, this.owner.getTeam()));
        }
    }
}
